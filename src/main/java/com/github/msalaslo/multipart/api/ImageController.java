...
public class ImageController {
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Images processed succesfully", content = {
			@Content(mediaType = "application/json", schema = @Schema(implementation = ImageAnalyticsResponse.class)) }),
			@ApiResponse(responseCode = "400", description = "Invalid request supplied", content = @Content),
			@ApiResponse(responseCode = "415", description = "Unsupported Media Type", content = @Content),
			@ApiResponse(responseCode = "500", description = "Internal error", content = @Content) })
	@PostMapping(value = "/images/set", consumes = { MediaType.MULTIPART_FORM_DATA_VALUE,
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Images processed succesfully", content = {
			...
			@ApiResponse(responseCode = "415", description = "Unsupported Media Type", content = @Content),
			@ApiResponse(responseCode = "500", description = "Internal error", content = @Content) })
	@PostMapping(value = "/images/set/options", consumes = { MediaType.MULTIPART_FORM_DATA_VALUE,
			MediaType.APPLICATION_OCTET_STREAM_VALUE }, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ImageAnalyticsResponse> analyzeImages(
			@RequestPart(value = "id", required = true) @Parameter(description = "The id of the image set") final String id,
			@RequestPart(value = "files", required = true) @Parameter(description = "Array of image files in binary format") final MultipartFile files[],
			@RequestPart(value = "processOptions", required = false) @Parameter(description = "ProcessOptions: Processing options", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE)) final ProcessOptions processOptions) {
		log.debug("Receiving images with id {}", id);
